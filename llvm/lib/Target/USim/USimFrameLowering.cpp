...
void USimFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(USim::RA);
    SavedRegs.set(USim::FP);
  }
  // Mark BP as used if function has dedicated base pointer.
  if (hasBP(MF))
    SavedRegs.set(USim::BP);
}

// TODO: Build insns
void USimFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  if (isInt<16>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(USim::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
...
void USimFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  auto *FI = MF.getInfo<USimFunctionInfo>();
  const USimRegisterInfo *RI = STI.getRegisterInfo();
  MachineBasicBlock::iterator MBBI = MBB.begin();

  Register FPReg = USim::FP;
  Register SPReg = USim::SP;
  // Register BPReg = USim::BP;
...
void USimFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  const USimRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *UFI = MF.getInfo<USimFunctionInfo>();
  Register FPReg = USim::FP;
  Register SPReg = USim::SP;
...
MachineBasicBlock::iterator USimFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == USim::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
bool USimFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
// TODO: rewrite!
StackOffset
USimFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
                                          Register &FrameReg) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo *RI = MF.getSubtarget().getRegisterInfo();
  const auto *UFI = MF.getInfo<USimFunctionInfo>();
...
bool USimFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
